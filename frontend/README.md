...
- **Caja** (`/caja`): abrir turno (monto inicial), cerrarlo (monto final) y registrar ingresos/egresos manuales, contra `POST /api/caja/abrir`, `POST /api/caja/{id}/cerrar` y `POST /api/caja/{id}/movimientos`. Visible para los tres roles (es lo primero que un vendedor necesita usar en su turno). Enter en el campo de monto confirma el modal (abrir o cerrar), sin tener que ir a buscar el botón con el mouse.
  - Al abrir "Cerrar caja" se consulta `GET /api/caja/{id}/comision` (comisión ganada en ese turno hasta ahora, sumando ventas y cobros de cuenta corriente de cada vendedor que operó la caja) y, si es mayor a cero, aparece un selector de cómo se le paga: "No pagar ahora", "Efectivo" o "Transferencia". Si se elige efectivo, ese monto se descuenta del efectivo esperado del turno (porque sale físicamente del cajón) — el backend recalcula la comisión en el momento del cierre, no confía en un monto mandado por el cliente.
- **POS** (`/pos`): pantalla de venta.
  ...
  - El botón "Pantalla del cliente" abre `/pantalla-cliente` en una ventana aparte, pensada para un segundo monitor de cara al cliente (con un botón para ponerla en pantalla completa). Se comunican entre sí con un `BroadcastChannel` del navegador (`src/pos/pantallaCliente.ts`) — no pasa por el backend ni necesita login, porque solo muestra lo mismo que ya se ve en el mostrador. El POS emite el carrito completo cada vez que cambia, y un mensaje aparte al confirmar la venta para que la pantalla del cliente muestre "Gracias por tu compra" con el total de esa venta (guardado en un estado propio, aparte del `total` general, porque el carrito ya se vació para cuando se termina de renderizar) antes de volver a un estado de espera.
- **Pantalla del cliente** (`/pantalla-cliente`, sin autenticación a propósito): ver arriba.
- **Productos** (`/productos`), **Insumos** (`/insumos`), **Clientes** (`/clientes`): alta/edición en un modal, mismo patrón que Proveedores. Productos permite además dar de baja (soft-delete) y crear una categoría nueva al vuelo desde el propio formulario.
  - Un producto vendido por peso siempre necesita un código PLU (y no puede tener código de barras fijo, porque la balanza lo cambia en cada pesada). Uno que NO se vende por peso puede tener código de barras, código PLU, o los dos — el PLU es para cuando la balanza está configurada para imprimirle una cantidad de unidades en vez de un peso (ver `backend/README.md`, sección de ventas, para el acuerdo de prefijos con la balanza real).
...
- **Reportes** (`/reportes`): un selector de rango de fechas y cinco pestañas — ventas por período (con desglose por día), ingresos por medio de pago, productos más vendidos, margen por producto, y stock crítico.
  - "Ingresos por medio de pago" grafica (con [Recharts](https://recharts.org/)) cuánto entró por día en efectivo/tarjeta débito/tarjeta crédito/transferencia, más un donut de efectivo vs. no efectivo — útil para decidir cuánto depositar en el banco. Sale de `GET /api/reportes/ingresos-por-medio-pago`, que suma ventas (menos las hechas a cuenta corriente, que todavía no son plata cobrada) más los cobros de cuenta corriente, que sí lo son y traen su propio medio de pago.
- **Comisiones** (`/comisiones`, solo `DUENO`): mismo selector de rango de fechas, con tablas de comisión de vendedores (por turno) y de producción, más un gráfico de barras con el total de comisión por vendedor en el período. La comisión de un vendedor se calcula sobre lo vendido **más** lo cobrado de cuenta corriente en ese turno — cobrar una deuda cuenta como gestión con el cliente, igual que una venta.
